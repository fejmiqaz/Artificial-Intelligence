...
if __name__ == '__main__':
    ...
    train_size = int((P/100) * len(dataset))
    ...
    for x_sample, y_sample in zip(x_test, y_test):
        predicted = nb_classifier.predict([x_sample])[0]
        if predicted == y_sample:
            nb_acc += 1

    nb_accuracy = nb_acc / len(x_test)

    print(nb_accuracy)

    decision_tree_classifier = DecisionTreeClassifier(criterion='entropy', random_state=0)
    decision_tree_classifier.fit(x_train, y_train)
    decision_predictor = decision_tree_classifier.predict(x_test)

    dt_acc = 0

    for x_sample, y_sample in zip(x_test, y_test):
        predicted = decision_tree_classifier.predict([x_sample])[0]
        if predicted == y_sample:
            dt_acc += 1
    decision_tree_accuracy = dt_acc / len(x_test)

    print(decision_tree_accuracy)

    random_forest_classifier = RandomForestClassifier(
        criterion='entropy', random_state=0, n_estimators=4
    )
    ...
    for x_sample, y_sample in zip(x_test, y_test):
        predicted = random_forest_classifier.predict([x_sample])[0]
        if predicted == y_sample:
            rf_acc += 1
    random_forest_accuracy = rf_acc / len(x_test)
    ...
    mlp_predictor = mlp_classifier.predict(x_test)

    mlp_acc = 0

    for x_sample, y_sample in zip(x_test, y_test):
        predicted = mlp_classifier.predict([x_sample])[0]
        if predicted == y_sample:
            mlp_acc += 1

    mlp_accuracy = mlp_acc / len(x_test)

    print(mlp_accuracy)

    classifiers = ["naive bayes", "decision tree", "random forest", "mlp"]
    accuracies = [nb_accuracy, decision_tree_accuracy, random_forest_accuracy, mlp_accuracy]

    max_accuracy = max(accuracies)
    best_idx = accuracies.index(max_accuracy)
    best_classifier = classifiers[best_idx]

    print("The highest accuracy was achieved by the classifier: ", best_classifier)
